...
def process_data_and_predict(age, weight, height, target):
    ...
    def prepare_training_data(category_data):
        fin_data = [] #store final training data
        for zz in range(5):
            for jj in range(len(category_data)):
                valloc = list(category_data[jj]) #converts array to list
    ...
    X_train_split, _, y_train_split, _ = train_test_split(X_train, [0] * len(X_train), test_size=0.2, random_state=42)
    ...
    y_pred = clf.predict(X_test_aligned) #lable the item 
...
def main():
    ...
    st.write("*This system is designed for people above age 18*")
    # Get user input
